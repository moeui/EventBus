interface IEvent {
  scope?: any, callback: Function, args: Array<any>
}

export class EventBus {
  events: {[key: string]: Array<IEvent>}
  constructor() {
      this.events = {};
  }
   ...
   */
  on(type: string, callback: Function, scope?: any, ...args: any[]) {
   ...
   */
  off(type: string, callback?: Function, scope?: any) {
      ...
      const filterFn = (event: IEvent) => event.scope !== scope || event.callback !== callback;
  ...
  has(type: string, callback: Function, scope: any): Boolean {
      ...
      const conditionFn = (event: IEvent): Boolean => {
